                            ...
                            @endphp
                                @foreach ($third_category as $item)
                                    <tr>
                                      ...
                                      <td>{{$item->name}}</td>
                                      <td>
                                        @if (isset($item->subCategory->category->name))
                                      ...
                                      <td><img src="{{asset('images/category/third_category/thumb/'.$item->image.'')}}" height="100" /></td>
                                      <td>
                                        @if ($item->status == '1')
                                      ...
                                      <td>
                                        <a href="{{route('admin.third_category_edit',['id'=>encrypt($item->id)])}}" class="btn btn-sm btn-warning">Edit</a>
                                        @if ($item->status == '1')
                                          <a href="{{route('admin.third_category_status',['id'=>encrypt($item->id),'status'=>2])}}" class="btn btn-sm btn-danger">Disable</a>
                                        @else
